...
class Sentence {
    ...
    public static function getExpressions($ids = []){
        $parts = [
            ...
            'regular-continuous-verb' => '\b(?!\b(during|wing|king|interesting|something|spring|hawking|thing|anything|everything|nothing|ceiling|building|dressing|dwelling|feeling|filling|longing|meaning|morning|evening|pudding|shilling|wedding)\b)(\w+ing)\b',
        ...
        $haveHasChars = [
            ['isWord' => 1, 'chars' => 'while'],
        ];
        $notBeforeGerundChars = [
            ['isWord' => 1, 'chars' => 'while'],
            ['isWord' => 1, 'chars' => 'when'],
            ['isWord' => 1, 'chars' => 'this'],
            ['isWord' => 1, 'chars' => 'her'],
            ...
            ['isWord' => 0, 'chars' => "'s"],
            ['isWord' => 0, 'chars' => "s'"],
//            ['isWord' => 1, 'chars' => 'that'], // that не може бути перед герундієм як "той мітинг...", але може бути перед герундієм як "що мітинг з друзями..."
        ];
// після while та when це partciple
        ...
        //  (?<!\ba\b)(?<!\bthe\b)\s(\w+ing) - ing, перед яким не стоїть артикль або possessive case
        //  (?<!a)(?<!the)\s(\w+ing) - article ing
        //  (?<!a)(?<!the)\s(\b[^\s]+\b)?(\s\w+ing) article someword ing

        $expressions = [];
        {
            $expression = '';
            foreach($notBeforeGerundChars as $item){
                // we need only ing word which is not followed by one of these words.
                // lets use look-behind
                if($item['isWord']){
                    $expression .= "(?<!\b{$item['chars']}\b)";
                } else {
                    $expression .= "(?<!{$item['chars']})";
                }
            }

            // lets find the ing word(it might not has space before if it's first word in the sentence)
            $expression .= '\s' . $parts['regular-continuous-verb'];
        ...
        {
            $expression = $parts['have-has'];
            $expression .= "(?=.*\b{$parts['indefinite-adverbs']}.*\b)(.{0,20})";
        }
...
//                })(),
//                'expression1' => implode('', [
////                    "(?!.*\b(the|a|am|is|are).*\b)(.{0,20})\b{$parts['regular-continuous-verb']}\b",
//                    "\b{$parts['regular-continuous-verb']}\b",
//                    "(?!(\.|,|!|;))(.{0,40})",
//                    "\b(is|was|has been|will be)\b"
//                ]),
//            ],
//            [
//                'id' => 2,
